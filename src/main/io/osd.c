...
static int osdNormalizeAngle(int angle)
{
    while (angle < 0) {
        angle += 360;
    }
...
void eraseSingleMapElementFromDisplay(osdMapElement_t * pOsdMapElements, int mapElementIndex)
{
    if (OSD_VISIBLE(pOsdMapElements[mapElementIndex].drawn)) {
        displayWriteChar(osdDisplayPort, OSD_X(pOsdMapElements[mapElementIndex].drawn), OSD_Y(pOsdMapElements[mapElementIndex].drawn), SYM_BLANK);
...
int getCountOfXYNeighbors(osdMapElementXYInfo_t * pOsdMapElementXYInfos, uint16_t osdMapElementXYCount, uint16_t poiX, uint16_t poiY)
{
	int neighborCount = 0;
    // Go through all the OSD Map Elements looking for given X,Y
    for (int i = 0; i < osdMapElementXYCount; i++) {
		if (pOsdMapElementXYInfos[i].poiX == poiX && pOsdMapElementXYInfos[i].poiY == poiY) {
			neighborCount++;
		}
	}
...
void markXYElementsAsNoLongerEligibleToDraw(osdMapElementXYInfo_t * pOsdMapElementXYInfos, uint16_t osdMapElementXYCount, uint16_t poiX, uint16_t poiY)
{
    // Go through all the OSD Map Elements looking for given X,Y
    for (int i = 0; i < osdMapElementXYCount; i++) {
		if (pOsdMapElementXYInfos[i].poiX == poiX && pOsdMapElementXYInfos[i].poiY == poiY) {
...
int getNeighborIndexForMultipleXYInSameLocation(osdMapElementXYInfo_t * pOsdMapElementXYInfos, uint16_t osdMapElementXYCount, uint16_t poiX, uint16_t poiY, int neighborCount)
{
    ...
    if (STATE(GPS_FIX)) {
        ...
        for (int i = 0; i < osdMapElementXYCount; i++) {
			...
			if (osdMapElementXYInfos[i].eligibleToBeDrawn) {
				...
				int neighborCount = getCountOfXYNeighbors(osdMapElementXYInfos, osdMapElementXYCount, osdMapElementXYInfos[i].poiX, osdMapElementXYInfos[i].poiY);
...
static void osdDrawRadar(uint32_t * pUsedScale)
{
...
static void osdDrawHomeMap(// Heading for the center element - what direction is up.
 						   // For radar, this will be the heading of the craft. 
						   // For map mode, center is home and map up is north.									 
...
{
    // There are 16 orientations for the home direction/craft rotation arrow.
    ...
    // Normalize to 0-360
    int rotatedArrowDir = osdNormalizeAngle(rotationInDegrees + 11);
    unsigned arrowOffset = rotatedArrowDir * 2 / 45;
    return SYM_ARROW_UP + arrowOffset;
}
    ...
    case OSD_HOME_DIR:
        {
            if (STATE(GPS_FIX) && STATE(GPS_FIX_HOME) && isImuHeadingValid()) {
                ...
                buff[0] = osdGetRotatedArrowCharacter(homeDirection);
            } else {
    ...
    case OSD_MAP_TAKEOFF:
        {
			...
			// -----------
            static uint32_t scale = 0;			
			osdDrawHomeMap(CENTIDEGREES_TO_DEGREES(navigationGetHomeHeading()), 'T', &scale);
            return true;
        }
    case OSD_RADAR:
        {
            ...
            // _NEW version
            // -------------
            // drawn is interior in the _NEW version
            static uint32_t scale = 0;
            osdDrawRadar(&scale);
            return true;
        }
    ...
    case OSD_WIND_SPEED_HORIZONTAL:
#ifdef USE_WIND_ESTIMATOR
        {
            ...
            if (valid) {
                ...
                int16_t windDirection = osdNormalizeAngle((int)angle - DECIDEGREES_TO_DEGREES(attitude.values.yaw));
...
void pgResetFn_osdConfig(osdConfig_t *osdConfig)
{
    ...
    osdConfig->map_scale_zoom_mode = OSD_MAP_SCALE_ZOOM_MODE_AUTO_RELATIVE;
    osdConfig->map_h_margin = 1;
    osdConfig->map_v_margin = 1;
}
